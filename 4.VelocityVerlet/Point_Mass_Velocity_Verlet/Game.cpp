...
bool cGame::InitializeGame()
{
	...
	SetupMatrices();
	// set vector gravity(0.0f,-9.81,0.0f) 
   ...
   // load ball one (ball1) calling Load() , the mesh filename is "bowlball.x" and the  device is Device;
	...
	// set its mass=10kg 
	ball1.mass = 10.0f;
	// set its initial position at location (5,1,3).that is also the location of the center of mass;
	...
	//do the same thing for ball two
	 // load ball two (ball2) calling Load(Filename) , the mesh filename is "bowlball.x" and the  device is Device;
...
void cGame::UpdateGame(float dt)
{
    static bool forceApplied=false;
	   ...
	   {     
		   ...
		   ball1.force = Vector3D(1.0f, 0.0f, 1.0f);
...
INT WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInst,LPSTR lpCmdLine, int nShowCmd)
{
 ...
 game=new cGame();
  /* Create window */
 game->CreateGameWindows(hInstance,"y  Velocity Verlet Method Simulation, Lab2");
